	...
	public class CastleBuilder : IBusContainerBuilder
	{
		...
		public void WithInterceptor(IConsumerInterceptor interceptor)
		{
			container.Kernel.ComponentModelCreated +=
				model =>
				{
					if (!typeof(IMessageConsumer).IsAssignableFrom(model.Implementation))
						return;
			...
			where T : class
		{
			...
			container.Register(
				Component.For<T>()
					.UsingFactoryMethod<T>(x => singleton ?? (singleton = func())));
		}
			...
			where T : class
		{
			...
			container.Register(
				Component.For<T>()
					.UsingFactoryMethod<T>(x => singleton ?? (singleton = func())).Named(name));
		}

		public void RegisterAll<T>(params Type[] excludes)
			where T : class
		{ RegisterAll<T>((Predicate<Type>)(x => !excludes.Contains(x))); }
